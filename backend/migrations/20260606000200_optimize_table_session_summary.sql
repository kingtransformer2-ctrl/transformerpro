-- Speed up live table-session billing summaries by replacing repeated
-- per-seat subqueries with session-scoped aggregate CTEs.
...
BEGIN
  ...
  session_items AS (
    SELECT
      oi.id,
      COALESCE(oi.seat_id, fs.id) AS resolved_seat_id,
      oi.total_price
