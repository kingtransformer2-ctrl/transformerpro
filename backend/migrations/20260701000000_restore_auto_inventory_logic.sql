-- Restore all automatic inventory deduction features
-- Reverts the changes from 20260626000000_remove_auto_inventory_logic.sql

DO $outer$
BEGIN
  -- Re-add inventory_deducted column
  IF NOT EXISTS (
    SELECT 1
  ...
  -- Recreate hotel_unlinked_order_flags table
  ...
  -- Enable RLS on hotel_unlinked_order_flags
  IF NOT EXISTS (
    SELECT 1
    FROM pg_tables
  ...
  -- Add policy for authenticated users to manage flags
  IF NOT EXISTS (
    SELECT 1
    FROM pg_policies
    WHERE schemaname = 'public'
  ...
  -- Create index on hotel_unlinked_order_flags
  ...
  -- Recreate all inventory automation functions
  ...
  -- Re-sync all service menu display stock
  DO $resync$
  DECLARE
    v_item RECORD;
  BEGIN
