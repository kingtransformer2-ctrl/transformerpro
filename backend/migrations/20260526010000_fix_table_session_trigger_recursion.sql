-- Prevent recursive table-session refresh triggers from exceeding PostgreSQL stack depth.
-- This fixes HTTP 500 failures from open_hotel_table_session / record_hotel_table_payment.
...
DECLARE
  v_session_id UUID;
BEGIN
  -- Guard against recursive trigger chains. The first trigger call still refreshes
  -- the session state, but any writes produced by that refresh do not re-enter it.
