-- Harden get_hotel_table_session_summary with SECURITY DEFINER
-- and ensure it can resolve sessions even if the status is not 'active'
-- (e.g. if the UI wants to show a closed session's final bill).
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM public.hotel_table_sessions WHERE id = p_session_id) THEN
    RETURN NULL;
  END IF;
