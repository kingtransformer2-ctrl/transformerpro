...
-- Fix existing unconfirmed users by confirming them if email confirmation is disabled
-- This will help users who are stuck
UPDATE auth.users 
SET 
...
-- Ensure profiles and roles exist for confirmed users who might be missing them
INSERT INTO public.profiles (user_id, email, first_name, last_name)
SELECT 
  au.id,
  au.email,
...
WHERE p.user_id IS NULL AND au.email_confirmed_at IS NOT NULL
ON CONFLICT (user_id) DO NOTHING;
...
WHERE ur.user_id IS NULL AND au.email_confirmed_at IS NOT NULL
ON CONFLICT (user_id) DO NOTHING;